...
db = SQLite3::Database.open "employee.db"      # opening sqlite data- employee database
db.execute "CREATE TABLE IF NOT EXISTS employees(id INTEGER PRIMARY KEY, first_name TEXT, last_name TEXT, salary INTEGER, active INTEGER)"  # executing that if it doesnt exist, creat id with integer, first_name, last_name, salary, and active

while true        # while this is correct
  system "clear"    # the system is clear
  results = db.query "SELECT * FROM employees"     # results are selectd from employee data 
  header = ["id", "first_name", "last_name", "salary", "active"]                # defines the data headers
  rows = results.to_a   #the results of query should be used
  table = TTY::Table.new header, rows   # a table is created
  puts "EMPLOYEES (#{rows.length} total)"  # employees is being put in all of the rows
  ...
  if input_choice == "c"   # if user inputs 'c'
    print "First name: " # prints first name - creates
    input_first_name = gets.chomp # gets first name input from user and creates
    print "Last name: "  # prints last name - creates
    input_last_name = gets.chomp # gets last name input from user
    print "Salary: " # prints salary
    input_salary = gets.chomp.to_i # user inputs their salary
    db.execute "INSERT INTO employees (first_name, last_name, salary, active) VALUES (?, ?, ?, ?)", [input_first_name, input_last_name, input_salary, 1] #the users input is inserted into table by forst employee
  elsif input_choice == "r" # prints out the database - reads
    print "Employee id: " # prints employee id
    input_id = gets.chomp.to_i # user inputs
    results = db.query "SELECT * FROM employees WHERE id = ?", input_id # selects employee data using user's id input
